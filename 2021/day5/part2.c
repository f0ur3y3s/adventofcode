...
main(int argc, char *argv[])
{
    ...
    while (fscanf(p_input_file, "%[^\n]\n", buffer) != EOF)
    {
        ...
        else if (abs(grid_x2 - grid_x1) == abs(grid_y2 - grid_y1))
        {
            // Diagonal line
            int increment_y = (grid_y1 < grid_y2) ? 1 : -1;
            int increment_x = (grid_x1 < grid_x2) ? 1 : -1;
            int x = grid_x1;
            int y = grid_y1;
            while (x != grid_x2 + increment_x && y != grid_y2 + increment_y)
            {
                p_grid->points[y][x].value += 1;
                x += increment_x;
                y += increment_y;
            }
        }

...
int is_possible_game (const game_t * target_game, char * line)
{
    ...
    int game_id = process_game_id(line);
    ...
    if (game->num_red <= target_game->num_red
        && game->num_green <= target_game->num_green
        && game->num_blue <= target_game->num_blue)
    {
        retval = game_id;
    }
...
int main (int argc, char * argv[])
{
    ...
    while ((read = getline(&line, &len, p_input_file)) > 0)
    {
        strsnl(line);
        int game_id = -1;
        game_id     = is_possible_game(target_game, line);

        if (0 == game_id)
        {
            printf("Game is not possible\n");
            continue;
        }
        else if (-1 == game_id)
        {
    ...
    printf("Sum of possible game_id: %d\n", sum);
    fclose(p_input_file);
    free(line);
}